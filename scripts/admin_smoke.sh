...
curl -s -b $JAR -c $JAR --max-time 60 \
    ...
    -d "_username=admin@quai-antique.test" \
...
echo "  contains 'Quai Antique' header: $(grep -c 'Quai Antique' /tmp/admin.html)"
echo "  contains 'Restaurants' menu: $(grep -c 'Restaurants' /tmp/admin.html)"

echo "==> /admin/restaurant (CRUD index)"
curl -sL -b $JAR -o /tmp/r.html -w "  %{http_code} %{size_download}b\n" --max-time 90 "$B/admin/restaurant"
echo "  contains 'Le Quai Antique' row: $(grep -c 'Le Quai Antique' /tmp/r.html)"

echo "==> /admin/dish (CRUD index)"
curl -sL -b $JAR -o /tmp/d.html -w "  %{http_code} %{size_download}b\n" --max-time 90 "$B/admin/dish"

echo "==> /account (authed cookie)"
curl -sL -b $JAR -o /tmp/acc.html -w "  %{http_code} %{size_download}b\n" --max-time 60 "$B/account"
echo "  contains admin email: $(grep -c 'admin@quai-antique.test' /tmp/acc.html)"
